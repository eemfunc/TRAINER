...
if($core->chk_GET(V_SEARCH_QUERY)){
    ...
    foreach($rows_sql as $r){
        if( ($search->v1($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1)) || 
            $search->v2($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1)) || 
            $search->v1($_GET[V_SEARCH_QUERY], $core->aes($r['item_no'], 1)) || 
            $search->v2($_GET[V_SEARCH_QUERY], $core->aes($r['item_no'], 1))) &&
            /* THE FOLLOWING IS TO HIDE EMPTY QUANTITY PRODUCTS WHEN CREATING A NEW INVOICE */
            $core->aes($core->dbFetch('products_quantities', array('product_id' => $r['id']))[0]['quantity'], 1) > 0
        ){
            array_push($all_items, array(
                'id'    => $core->aes($r['name'], 1).' ('.$r['id'].')',
