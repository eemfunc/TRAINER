...
if($core->chk_GET(V_SEARCH_QUERY)){
    ...
    $total_rows = 0;
    $rows_sql = $core->dbFetch('customers', null, 'ORDER BY created_at DESC');
    foreach($rows_sql as $r){
        if($search->v1($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1)) || $search->v2($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1))){
            $c = "";
            if($core->aes($r['city'], 1) == "بغداد" || $core->aes($r['city'], 1) == ""){
                if($core->aes($r['region'], 1) != "")
                    $c = " (".$core->aes($r['region'], 1).")";
            }else 
                $c = " (".$core->aes($r['city'], 1).")";
            array_push($all_items, array(
                'id'    => $core->aes($r['name'], 1).' ('.$r['id'].')',
                'text'  => $core->aes($r['name'], 1).$c
            ));
            $total_rows++;
        }
    }
