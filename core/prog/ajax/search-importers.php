...
if($core->chk_GET(V_SEARCH_QUERY)){
    ...
    $total_rows = 0;
    $rows_sql = $core->dbFetch('importers', null, 'ORDER BY created_at DESC');
    ...
    if($total_rows<$start_from+$rows)$start_to=$total_rows;else $start_to=$start_from+$rows;
    if($total_rows>0)
