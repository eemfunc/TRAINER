...
if($core->chk_GET(V_SEARCH_QUERY)){
    ...
    foreach($rows_sql as $r){
        if($search->v1($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1)) || $search->v2($_GET[V_SEARCH_QUERY], $core->aes($r['name'], 1)) || $search->v1($_GET[V_SEARCH_QUERY], $core->aes($r['item_no'], 1)) || $search->v2($_GET[V_SEARCH_QUERY], $core->aes($r['item_no'], 1))){
            ...
            $p_r = $core->numCur($r['price_retail'], 'USD', true);
            $temp = $core->numCur($r['price_retail_iqd'], 'IQD', true);
            if($temp != '')
                $p_r = $temp;
            elseif($p_r == '')
                $p_r = $core->txt('0120');
            
            $p_w = $core->numCur($r['price_wholesale'], 'USD', true);
            $temp = $core->numCur($r['price_wholesale_iqd'], 'IQD', true);
            if($temp != '')
                $p_w = $temp;
            elseif($p_w == '')
                $p_w = $core->txt('0120');
            
            $p_d = $core->numCur($r['price_distribution'], 'USD', true);
            if($p_d == '')
                $p_d = $core->txt('0120');
            
            array_push($all_items, array(
                'id'    => $r['id'],
                'text'  => $core->aes($r['name'], 1). ' ('.$core->aes($r['item_no'], 1).') --- ('.$p_r.' | '.$p_w.' | '.$p_d.')'
            ));
            $total_rows++;
        }
    }
